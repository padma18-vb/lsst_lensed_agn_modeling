...
print('subtract_lens: ', subtract_lens)
print('subtract_source: ', subtract_source)
psf_kernels = np.load('data/norm_resize_psf.npy')
...
config_dict = {
	'main_deflector':{
		...
		'parameters':{
			'z_lens': None,
			'gamma': norm(loc=2.035,scale=0.166).rvs,
			'theta_E': norm(0.875,scale=0.336).rvs,
			'e1':None,
			'e2':None,
			'center_x': None,
			'center_y': None,
			'gamma1': norm(loc=-0.001, scale=0.042).rvs,
			'gamma2': norm(loc=-0.0003, scale=0.043).rvs,
			'ra_0':0.0, 'dec_0':0.0
		}
	},
	'lens_light':{
	...
	'source':{
		...
		'parameters':{
			'z_source':None,
			'mag_app':norm(loc=23.82, scale = 1.23).rvs,
			'output_ab_zeropoint':output_ab_zeropoint,
			'R_sersic':norm(loc=0.538,scale=0.287).rvs,
    ...
    'cross_object':{
		'parameters':{
            ('main_deflector:center_x,lens_light:center_x'):dist.DuplicateScatter(
                dist=norm(loc=0.002,scale=0.05).rvs,scatter=0.001),
            ('main_deflector:center_y,lens_light:center_y'):dist.DuplicateScatter(
                dist=norm(loc=0.0002,scale=0.05).rvs,scatter=0.001),
			('main_deflector:e1,lens_light:e1'):dist.DuplicateScatter(
                dist=norm(loc=-0.004,scale=0.15).rvs,scatter=0.5),
            ('main_deflector:e2,lens_light:e2'):dist.DuplicateScatter(
                dist=norm(loc=-0.003,scale=0.15).rvs,scatter=0.5),
