...
config_dict = {
	'main_deflector':{
		...
		'parameters':{
			'z_lens': None,
			'gamma': norm(loc=2,scale=0.2/2).rvs,
			'theta_E': truncnorm(-0.5, np.inf, loc=0.8,scale=1/2).rvs,
			'e1':None,
			'e2':None,
			'center_x': None,
			'center_y': None,
			'gamma1': norm(loc=0, scale=0.1/2).rvs,
			'gamma2': norm(loc=0, scale=0.1/2).rvs,
			'ra_0':0.0, 'dec_0':0.0
		}
	},
	'lens_light':{
	...
	'source':{
		...
		'parameters':{
			'z_source':None,
			'mag_app':norm(loc=24, scale = 2/2).rvs,
			'output_ab_zeropoint':output_ab_zeropoint,
			'R_sersic':truncnorm(-0.5, np.inf, loc=0.7,scale=1/2).rvs,
