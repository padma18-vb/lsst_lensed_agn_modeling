...
config_dict = {
	'main_deflector':{
		'class': PEMDShear,
		'file': os.path.join(root_path, 'data/deflectors3.csv'),
		'parameters':{
			'z_lens': 'ZLENS',
			#'gamma':2,
			...
			'gamma1': norm(loc=-0.001, scale=0.042).rvs, # added to catalog
			'gamma2': norm(loc=-0.0003, scale=0.043).rvs, # added to catalog
			'ra_0':0.0, 'dec_0':0.0,
		}
	},
	'lens_light':{
	...
	'source':{
		...
		'parameters':{
			'z_source': 'redshift',
			'mag_app': norm(loc=23.82, scale = 1.23).rvs, # SOURCE APPARENT MAG
			'output_ab_zeropoint':output_ab_zeropoint,
